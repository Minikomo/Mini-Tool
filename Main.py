...
def main():
    ...
    print()
    print(
        """[1] Server Joiner             [2] Channel Spammer           [3] ID Scraper"""
    )
    print(
        """[4] Message Reactor           [5] Discord verify           [3] ID Scraper"""
    )
    print()
    global mode
    mode = tinput("Mode")
...
if mode == "1":
    ...
    class Joiner:
        def __init__(self):
            ...
            ts = [
                threading.Thread(
                    target=self.join, args=[token, invite_code, self.proxy()]
                )
                for token in tokens
            ]
            for t in ts:
        ...
        def join(self, token, invite, proxy):
            ...
            req = self.session.post(
                f"https://discord.com/api/v9/invites/{invite}", json={}, headers=headers
            )
        ...
        def xheaders(self):
            ...
            return base64.b64encode(xconst.encode("utf-8")), base64.b64encode(
                xprops.encode("utf-8")
            )
    ...
    def closefetching(resp, guildid):
        if bot.gateway.finishedMemberFetching(guildid):
            ...
            bot.gateway.removeCommand(
                {"function": closefetching, "params": {"guildid": guildid}}
            )
            bot.gateway.close()
...
if mode == "5":
    ...
    def verify(token):
        ...
        jasuoon = requests.get(
            f"https://discord.com/api/v9/guilds/{serverid}/member-verification",
            headers=headers,
        )
        ...
        verify = requests.put(
            f"https://discord.com/api/v9/guilds/{serverid}/requests/@me",
            headers=headers,
            json=json_data,
        )
